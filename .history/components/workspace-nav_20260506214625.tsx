...
const secondaryModules = ["Messages", "Calendar", "Kanban", "Product list", "Users"];
    ...
    <>
      <aside className="glass-panel-strong hidden min-h-full w-full max-w-[320px] flex-col rounded-[28px] p-5 lg:flex">
        ...
        <section className="mt-4 rounded-[28px] border border-white/10 bg-white/5 p-4">
          <div className="text-[11px] uppercase tracking-[0.3em] text-[var(--brand-peach)]">Modules</div>
          <div className="mt-3 flex flex-wrap gap-2">
            {secondaryModules.map((item) => (
              <span key={item} className="pill rounded-full px-3 py-2 text-xs text-white">
                {item}
              </span>
            ))}
          </div>
        ...
        <section className="mt-4 rounded-[28px] border border-white/10 bg-white/5 p-4">
          <div className="text-[11px] uppercase tracking-[0.3em] text-[var(--brand-peach)]">Messages</div>
          <div className="mt-3 space-y-3">
            {messageContacts.map((contact) => (
              <div key={contact.name} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[rgba(255,255,255,0.12)] text-xs font-semibold text-white">
                  {initials(contact.name)}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm text-white">{contact.name}</div>
                  <div className="text-xs text-[var(--muted)]">{contact.status}</div>
                </div>
        ...
        <section className="mt-4 rounded-[28px] border border-white/10 bg-white/5 p-4">
          <div className="text-[11px] uppercase tracking-[0.3em] text-[var(--brand-peach)]">Live status</div>
          <div className="mt-3 space-y-2 text-sm leading-6 text-[var(--muted)]">
            <div>Session auth and audit trail ready</div>
            <div>Role-aware nav visible on every workspace page</div>
            <div>Desktop sidebar + mobile drawer active</div>
          </div>
        </section>
      </aside>
