  ...
  return (
    ...
    >
      <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="glass-panel rounded-[28px] p-5 sm:p-7">
          <SectionHeader title="Scoring workspace" subtitle="Use the same rubric the reviewer uses in production." />
          <EvaluationForm agentName={data.agents[0]?.name ?? "Selected agent"} />
        </div>
      </section>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {workspaceCases.assigned.map((qualityCase) => (
            <div key={qualityCase.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <button className="brand-gradient mt-4 w-full rounded-2xl px-4 py-3 text-sm font-semibold text-white">
                Publish case
              </button>
            </div>
          ))}
